...
use vars qw( $S_CODE $S_COMMENT $S_DOC_COMMENT $S_LINE_COMMENT $S_STRING $S_PROTOTYPE);
# general scanner modes
@scan_mode_names = qw( CODE COMMENT DOC_COMMENT LINE_COMMENT STRING PROTOTYPE );
$S_CODE = 0;
$S_COMMENT = 1;
$S_DOC_COMMENT = 2;
$S_LINE_COMMENT = 3;
	...
	LOOP: while( $scan_mode == $S_DOC_COMMENT )
	{
		if( $token =~ /(?:\\|@)/ )
		{
			...
			else
			{
				...
				/^(?:\\|@)public$/ && do
				{
					warning( "Visibility already defined!" )
						if( exists $doc->{visibility} );
				...
				{
                    my $type;
					( $type, $token ) = parse_type();
                    push @{$doc->{implements}}, $type;
...
sub parse_prototype
{
	...
	if( $member eq "" )
	{
        ...
        if ($token eq "{") {
            ...
            PARSEPROTO: while(( $token = next_parser_token()) ne "" )
            {
                if( $scan_mode == $S_CODE )
                {
                    for( $token )
                    {
                        /^}$/
                            && do { last PARSEPROTO if( --$my_level < 0 ); };
                        /^{$/				
                            && do { ++$my_level; next; };
                        (/^get$/ || /^set$/) && $my_level == 0
                            && do {
                                ...
                                if ($token =~ /^[_[:alpha:]]\w*$/) {
                                    syntax_err( "Invalid getter definition." )
                                        if next_parser_token() ne '(';
                                    syntax_err( "Invalid getter definition." )
                                        if next_parser_token() ne ')';
                                    ...
                                    my $base = parse_variable( $context );
                                    my $scope = delete $context->{objs}{$token};

                                    $scope->{otype} = $OT_MEMBERVAR;
                                    $fnContext->{members}{$token} = $scope;
                                }
                                next; };
                        /^:$/ && $my_level == 0
                            && do {
                                $token = next_parser_token();
                                $my_level++, next if ($token eq '{');
                                push @parser_stack, $token;
                                ...
                                if ($token eq "function") {
                                    ...
                                    unshift @token_queue, ";";
                                    unshift @token_queue, ";";
                                    push @parser_stack, $id_token;

                                    my $base = parse_function( $context );
                                    my $scope = $context->{objs}{$base};

                                    $scope = $scope->{scope} 
                                        while( $scope and ( not exists $scope->{objs}{$base} ));

                                    syntax_err( "Invalid member definition." ) if not $scope;

                                    $scope = delete $scope->{objs}{$base};

                                    $scope->{otype} = $OT_MEMBERFUNC;
                                    $fnContext->{members}{$id_token} = $scope;
                                } elsif ($id_token =~ /^[_[:alpha:]]\w*$/) {
                                    push @parser_stack, '=';
                                    push @parser_stack, $id_token;

                                    my $base = parse_variable( $context );
                                    my $scope = delete $context->{objs}{$id_token};

                                    $scope->{otype} = $OT_MEMBERVAR;
                                    $fnContext->{members}{$id_token} = $scope;
                                }
                                next; };
                        /^__proto__$/ && $my_level == 0
                            && do {
                                $id_token = $token;
                                syntax_err( "Invalid prototype definition." )
                                    if (next_parser_token() ne ":");

                                my $base = parse_code();
                                syntax_err( "Invalid prototype definition." )
                                    if ($token eq "");
                                while(( $token = parse_code ) =~ /\.|prototype/ ) {}
                                syntax_err( "',' expected, found '$token'." )
                                    if( $token !~ /^[,}]$/);
                                $prev_token = $token;
                                ...
                                debug_msg( $DEB_DATABASE,
                                    "Set '$base' as base for class '$name'." );
                                if( $last_doc )
                                {
                                    if( exists $fnContext->{doc} )
                                    {
                                        warning( 
                                            "Comment for '$name' already exists, ignoring new." );
                                    }
                                    else
                                    {
                                        $fnContext->{doc} = $last_doc;
                                ...
                                $last_doc = undef;
                                last PARSEPROTO if( $prev_token eq "}" && --$my_level < 0 );
                                next; };
                    }
                    $prev_token = $token;
                }
            ...
            syntax_err( "Unbalanced '}' found." ) if( $my_level < -1 );
            syntax_err( "EOF found. '}' expected." ) 
                if( $my_level >= 0 && $context->{otype} != $OT_FILE );
        } else {
...
sub generate_function
{
    ...
    print "$visibility:\n" if ($func->{otype} == $OT_MEMBERFUNC and not $npref);
    print "static " if $static;
...
sub generate_variable
{
	...
	if( exists $var->{doc} )
	{
        ...
        $var->{otype} = $OT_MEMBERSVAR if $var->{otype} == $OT_MEMBERVAR &&
            $var->{doc}{static};
		print( "\n".$var->{doc}{text} );
		$rtype = $var->{doc}{rtype} if( exists $var->{doc}{rtype} );
        $visibility = $var->{doc}->{visibility};
        $static = $var->{doc}->{static};
	}
    $visibility = $visibility || "private";
    print "$visibility:\n" if ($var->{otype} == $OT_MEMBERVAR ||
        $var->{otype} == $OT_MEMBERSVAR);
    print "static " if $static;
...
sub generate_class
{
	...
	else
	{
        ...
        if ($context->{doc}) {
    		for my $if( @{$context->{doc}->{implements}} ) {
        		my $scope = create_base( $context, $if );
		        $scope = $scope->{objs}{$if};
        		syntax_err( "$if is a '"
		    		.$object_type_names[$scope->{otype}]."', but not a class." )
        			if( $scope->{otype} != $OT_INTERFACE && $scope->{otype} != $OT_CLASS);
